...
# Dehost-enabled derivative of the canonical metage_v2.88.3 workflow.
# Resource configuration is kept synchronized with the canonical workflow.


workflow metage_v2_88_3_dehost {
    ...
    # 不传时保持 v2.88.3 原有默认样式。布尔值和数值使用 String 是为了兼容
    ...
    if(run_mode != "full"){
        ...
        call deal_parameter {
            input:
                workflow_dir="/cephfs_data/genostack_v3/genostack_cromwell/cromwell-executions/metage_v2_88_3/" + select_first([parent_workflow_dir])
        }
    }
...
task deal_parameter{
    ...
    >>>
    runtime {
        docker:"dockerhub.genostack.com/sanshu/metage:v2.88.3"
        cpu:"12"
        memory:"20 GB"
    }
    output {
