# metage_v2.88.3：node1 测试包

本包用于在 node1 测试正式发布的 `metage_v2.88.3`。运行脚本已经打入 Docker
镜像，因此不重复打包 `scripts/`；本包也不包含 FASTQ、数据库、Docker tar 或
Cromwell 历史结果。

## 固定版本

- WDL：`workflow/metage_v2.88.3.wdl`（普通版）和
  `workflow/metage_v2.88.3_dehost.wdl`（去宿主版）
- 发布标签：`dockerhub.genostack.com/sanshu/metage:v2.88.3`
- 当前 RepoDigest：`dockerhub.genostack.com/sanshu/metage@sha256:e8d824b1727608a54e943c79b826760d258637c280384d21f7e31a88227b4f9a`
- Image ID：`sha256:5890fde7b80c447fc21f8a1b5a349dcc2f56eaed28ddbba4248d7fbad5efb001`
- 创建时间：`2026-08-26T23:03:40.633380121+08:00`
- 架构：`amd64`

WDL 的常规 task 均使用发布标签 `metage:v2.88.3`；两个分箱 task 继续使用独立的
...
```text
node1_test_bundle_v2.88.3/
├── workflow/
│   ├── metage_v2.88.3.wdl                  # 普通版
│   └── metage_v2.88.3_dehost.wdl           # 去宿主版
├── inputs/
...
├── docs/
│   ├── metage_v2.88.3_WDL参数说明.xlsx
│   └── metage_v2.88.3_参数说明.txt
...
1. 普通流程上传 `workflow/metage_v2.88.3.wdl`；去宿主流程上传
   `workflow/metage_v2.88.3_dehost.wdl`
