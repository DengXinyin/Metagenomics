...
REQUIRED_OUTPUTS = (
    ...
    "call-kneaddata_no/execution/Result",
    "call-megahit_no/execution/megahit",
    "call-prodig_no/execution/prodigal",
    "call-bwa_no/execution/bowtie",
    "call-tax_anno/execution/Annotation",
    "call-func_anno/execution/Annotation",
    "call-anno/execution/Annotation",
    "call-VCA_anno/execution/ARGs",
    "call-VCA_anno/execution/CycDB",
    "call-VCA_anno/execution/VFDB",
    "call-MBQ_anno/execution/BacMet2",
    "call-MBQ_anno/execution/QS",
    "call-MBQ_anno/execution/mobileOGs",
    "call-COG_anno/execution/COG",
    "call-MetaCyc_anno/execution/MetaCyc",
)
...
def valid_parent(workflow_dir):
    return workflow_dir.is_dir() and all((workflow_dir / rel).is_dir() for rel in REQUIRED_OUTPUTS)


def discover_registries(workspace_root, current_registry_dir, identity):
    paths = set(current_registry_dir.glob("*_sample_registry.tsv"))
    paths.update(workspace_root.glob(f"*/cromwell-executions/{WORKFLOW_NAME}/registry/*_sample_registry.tsv"))
...
def discover_workflows(workspace_root, registries, identity):
    candidates = set()
    for registry in registries:
        candidates.update(workflows_from_registry(registry))
    candidates.update(workspace_root.glob(f"*/cromwell-executions/{WORKFLOW_NAME}/*"))
...
def parent_samples(workflow_dir):
    if not workflow_dir:
        return set()
    megahit = workflow_dir / "call-megahit_no/execution/megahit"
...
def choose_mode(requested, has_parent, changes, inputs):
    ...
    if str(input_value(inputs, "binning", "no")).lower() == "yes":
        ...
        return "full"
    if bool(input_value(inputs, "use_kraken2", False)) or str(input_value(inputs, "ref_sample", "")).strip():
...
def main():
    ...
    registries = discover_registries(workspace_root, registry_dir, identity)
    workflows = discover_workflows(workspace_root, registries, identity)
    parent = workflows[0] if workflows else None
    ...
    plan = {
        ...
        "parent_workflow_dir": str(parent) if parent else "",
