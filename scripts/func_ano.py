...
def eggNOG_mapper(emapperdir, prodigal_dir, dbdir, anno_dir):
    cmd = '''
python {0}/emapper.py --cpu 60 -i {1}/unique_gene.fasta \
--itype CDS -m diamond --evalue 1e-5 \
...
def func_table(bowtie, anno_dir, mapdir, dbdir):
    gene_tpm = pd.read_csv('%s/gene_tpm.csv' % bowtie)
    ano_all = pd.read_csv('%s/func.emapper.annotations' % anno_dir, sep='\t')
...
def main():
    ...
    func_table(bowtie, anno_dir, mapdir, dbdir)
