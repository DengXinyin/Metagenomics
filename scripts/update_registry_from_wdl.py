...
def update_registry(registry_df, project_info, file_records, filter_project_no=None,
    ...
    1. 优先使用已有 registry 中的非空 display_name/group。
    2. 如果 registry 中没有，则尝试从 workflow 执行目录中的 sample-metadata.tsv 回填。
    3. 更新后，对所有相同 internal_id 的 registry 记录进行 group/display_name 回填。
    """
    ...
    # 为 internal_id 建立 display_name/group 映射：优先 registry 中的非空值，再回退到 metadata
    display_map = {}
    group_map = {}
    ...
    # 对已有记录回填 group/display_name（registry 中原本为空，但 metadata 或新记录提供了）
    if not merged.empty and "internal_id" in merged.columns:
        for iid, grp in group_map.items():
            if not iid or not grp:
                continue
            mask = (merged["internal_id"] == iid) & (merged["group"].isna() | (merged["group"] == ""))
            if mask.any():
                merged.loc[mask, "group"] = grp
        for iid, disp in display_map.items():
            if not iid or not disp:
                continue
            mask = (merged["internal_id"] == iid) & (merged["display_name"].isna() | (merged["display_name"] == ""))
            if mask.any():
...
def reorder_columns(registry_df):
    """调整列顺序。"""
    base_cols = ["internal_id", "display_name", "group", "project_no", "project_name", "customer_name",
                 "task", "file_path", "description", "status"]
    ordered = [c for c in base_cols if c in registry_df.columns]
    ordered += [c for c in registry_df.columns if c not in base_cols]
