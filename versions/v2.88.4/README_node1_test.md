# metage_v2.88.4：node1 测试包

本包用于在 node1 测试正式发布的 `metage_v2.88.4`。运行脚本已经打入 Docker
镜像，因此不重复打包 `scripts/`；本包也不包含 FASTQ、数据库、Docker tar 或
Cromwell 历史结果。

## 固定版本

- WDL：`workflow/metage_v2.88.4.wdl`（普通版）和
  `workflow/metage_v2.88.4_dehost.wdl`（去宿主版）
- 发布标签：`dockerhub.genostack.com/sanshu/metage:v2.88.4`
- 当前 RepoDigest：`dockerhub.genostack.com/sanshu/metage@sha256:510f5919a4a4b926708884db5671ed9196d82469fc00f2f0ea74afd9b965c5d3`
- Image ID：`sha256:0ae59e35bc460c91e0bb285e2c528ad0bf5f6d59660b06d1db78deacecff5b48`
- 创建时间：`2026-08-30T23:28:57.823140865+08:00`
- 架构：`amd64`

WDL 的常规 task 均使用发布标签 `metage:v2.88.4`；两个分箱 task 继续使用独立的
...
```text
node1_test_bundle_v2.88.4/
├── workflow/
│   ├── metage_v2.88.4.wdl                  # 普通版
│   └── metage_v2.88.4_dehost.wdl           # 去宿主版
├── inputs/
...
├── docs/
│   ├── metage_v2.88.4_WDL参数说明.xlsx
│   └── metage_v2.88.4_参数说明.txt
...
1. 普通流程上传 `workflow/metage_v2.88.4.wdl`；去宿主流程上传
   `workflow/metage_v2.88.4_dehost.wdl`
