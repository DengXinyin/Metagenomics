...
def ensure_bracken_kmer_dist(db_dir, read_length, threads=8, kmer_len=35):
    """
    确保 bracken 的 kmer distribution 文件存在；不存在则调用 bracken-build 生成。
    """
    ...
    log.info('bracken kmer distribution 文件不存在，开始构建: %s', dist_file)
    # 检测 kraken2 数据库的 kmer 长度（hash.k2d 所在目录的 k 值）
    # 多数标准库 k=35，MiniKraken2 也是 35
    cmd = 'bracken-build -d {db} -t {t} -k {k} -l {l}'.format(
        db=db_dir, t=threads, k=kmer_len, l=read_length
    )
    run_cmd(cmd)
    return read_length
...
def main():
    ...
    # 确保 bracken kmer distribution 已生成
    ensure_bracken_kmer_dist(db_dir, read_length, threads=args.threads)
