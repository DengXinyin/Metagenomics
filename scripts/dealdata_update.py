...
# 兼容 comparison sheet 仅含表头、没有数据行的情况（表头本身描述分组）
if df_comp.empty and len(df_comp.columns) > 1:
    df_comp = pd.DataFrame([df_comp.columns.tolist()], columns=df_comp.columns)
    print("⚠️ comparison sheet 为空，已使用列名作为分组信息")

# 生成 sample.txt
sample_path = os.path.join(args.outdir, "sample.txt")
df_sample[["fastqfile", "sample"]].to_csv(sample_path, sep="\t", index=False)

# 构建 group_map：key 保留原始分组名，value 为成员类别列表
raw_group_map = {}
for _, row in df_comp.iterrows():
    group_name = row.iloc[0]
    members = row.iloc[1:].dropna().astype(str).tolist()
    if members:
        raw_group_map[group_name] = members

# 下游脚本（QC_stats、tax_stats 等）期望 metadata 列名为 group1, group2...
groups = [f"group{i+1}" for i in range(len(raw_group_map))]
group_map = dict(zip(groups, raw_group_map.values()))
