...
KRAKEN_SCRIPT = Path(__file__).with_name("merge_kraken2_results.py")
...
def batch(root, samples):
    ...
    write(root / "tax_annotation/Tax_id.tmp.txt", "GeneID\ttaxid\n" + "".join("%s\ttax_%s\n" % (g, g) for g in genes))
...
def assert_kraken_incremental_merge(root):
    data = root / "kraken_data"
    metadata(data, ["A", "C"])
    old, new, output = root / "kraken_old", root / "kraken_new", root / "kraken_merged"
    for base, samples, marker in ((old, ["A", "B"], "old"), (new, ["A", "C"], "new")):
        for sample in samples:
            write(base / sample / (sample + ".kreport2.txt"), marker + "\n")
            write(base / sample / (sample + ".S.bracken.txt"), marker + "\n")
    subprocess.run([
        sys.executable, str(KRAKEN_SCRIPT), "--old", str(old), "--new", str(new),
        "--datapath", str(data), "--out", str(output),
    ], check=True)
    assert sorted(path.name for path in output.iterdir()) == ["A", "C"]
    assert (output / "A/A.kreport2.txt").read_text(encoding="utf-8") == "new\n"

    changes = {"added": ["C"], "deleted": [], "renamed": [], "regrouped": [], "fastq_changed": []}
    assert planner.choose_mode("add", True, changes, {"use_kraken2": "yes"}) == "incremental"
...
def assert_state(root, samples):
    ...
    assert (root / "tax_annotation/Tax_id.tmp.txt").read_text(encoding="utf-8").splitlines()[0] == "GeneID\ttaxid"
...
def main():
    with tempfile.TemporaryDirectory(prefix="metage_incremental_test_") as temporary:
        ...
        assert_kraken_incremental_merge(root)
