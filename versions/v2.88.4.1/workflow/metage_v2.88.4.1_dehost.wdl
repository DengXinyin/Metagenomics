...
# Dehost-enabled derivative of the canonical metage_v2.88.4.1 workflow.
# Resource configuration is kept synchronized with the canonical workflow.


workflow metage_v2_88_4_1_dehost {
    ...
    # 不传时保持 v2.88.4.1 默认样式。布尔值和数值使用 String 是为了兼容
    ...
    if (run_mode == "incremental") {
        ...
        if (use_kraken2 == "yes") {
            call merge_kraken2_results {
                input:
                    workflow_dir=select_first([parent_workflow_dir]),
                    new_kraken2=select_first([kraken2_anno.kraken2_out]),
                    datapath=select_first([check_input_no_raw.result])
            }
        }
    ...
    if (use_kraken2 == "yes" && (run_mode == "full" || run_mode == "incremental")) {
        call kraken2_tax_base {
            input:
                datapath=apply_registry.new_datapath,
                kraken2_out=if run_mode == "incremental" then select_first([merge_kraken2_results.kraken2_out]) else select_first([kraken2_anno.kraken2_out]),
                plot_style=choose_plot_style.plot_style
        }
        call kraken2_tax_diff {
            input:
                datapath=apply_registry.new_datapath,
                preResdir=kraken2_tax_base.Result,
    ...
    if (binning == 'yes'){
            call coll_res_ana_bins {
                input:
                    ...
                    kraken2_anno_dir=if run_mode == "incremental" then merge_kraken2_results.kraken2_out else kraken2_anno.kraken2_out,
    ...
    output {
        ...
        File? kraken2_out = if run_mode == "incremental" then merge_kraken2_results.kraken2_out else kraken2_anno.kraken2_out
...
task deal_parameter{
    ...
    command <<<
        ...
        PARENT_ID="${workflow_dir}"
        ROOT=""
        for base in \
            /cephfs_data/genostack_v3/genostack_cromwell/cromwell-executions/metage_v2_88_4_1_dehost \
            /cephfs_data/genostack_v3/genostack_cromwell/cromwell-executions/metage_v2_88_4_1 \
            /cephfs_data/genostack_v3/genostack_cromwell/cromwell-executions/metage_v2_88_4_dehost \
            /cephfs_data/genostack_v3/genostack_cromwell/cromwell-executions/metage_v2_88_4
        do
            if [ -d "$base/$PARENT_ID" ]; then ROOT="$base/$PARENT_ID"; break; fi
        done
        if [ -z "$ROOT" ]; then
            echo "ERROR: parent workflow not found: $PARENT_ID" >&2
            exit 2
        fi
    ...
    >>>
    runtime {
        docker:"dockerhub.genostack.com/sanshu/metage:v2.88.4.1"
        cpu:"12"
        memory:"20 GB"
    }
    output {
...
task merge_kraken2_results {
    ...
    command <<<
        set -euo pipefail
        echo "merge_kraken2_results version: incremental-v1"
        PARENT_ID="${workflow_dir}"
        ROOT=""
        for base in \
            /cephfs_data/genostack_v3/genostack_cromwell/cromwell-executions/metage_v2_88_4_1_dehost \
        ...
        OLD=""
        for candidate in \
            "$ROOT/call-merge_kraken2_results/execution/kraken2_out" \
            "$ROOT/call-merge_kraken2_results/cacheCopy/execution/kraken2_out" \
            "$ROOT/call-kraken2_anno/execution/kraken2_out" \
            "$ROOT/call-kraken2_anno/cacheCopy/execution/kraken2_out"
        do
            if [ -d "$candidate" ]; then
                OLD="$candidate"
                break
        ...
        if [ -z "$OLD" ]; then
            echo "ERROR: parent Kraken2 result missing under $ROOT" >&2
            exit 2
        fi
        /root/anaconda3/envs/py39/bin/python \
            /root/microbiome/microbiome/metage_v2.88.2/merge_kraken2_results.py \
            --old "$OLD" \
            --new ${new_kraken2} \
            --datapath ${datapath} \
            --out kraken2_out
