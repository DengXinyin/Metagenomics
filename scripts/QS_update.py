...
def get_qs_table(dbdir, qs_dir, anno_dir, bowtie):
    ...
    if qs_dat.empty:
        ...
        output_cols = ['GeneID', 'taxonomy', 'Entry', 'Reviewed', 'Entry Name', 'Gene Names',
                       'Length', 'Protein names', 'Function [CC]', 'Protein family'] + list(gene_tpm.columns[1:])
        pd.DataFrame(columns=output_cols).to_csv('%s/QS.tpm.csv' % qs_dir, index=False, encoding='utf-8-sig')
        return
